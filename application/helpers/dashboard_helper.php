...
{
    function input_date($name,$row_name=null)
    {
      $form = "";
      $form .= "<div class=\"form-group row mb-4\">
                  <label for=\"".$name."\" class=\"col-form-label text-md-right col-12 col-md-3 col-lg-3\">".ucfirst(str_replace('_',' ', $name))."</label>
                  <div class=\"col-sm-12 col-md-7\"> 
                    <input id=\"".$name."\" type=\"date\" class=\"form-control\" ".error_border(form_error($name))." placeholder=\"Buat ".ucfirst(str_replace('_', ' ', $name))."\" value=\"".set_value($name,$row_name)."\" name=\"".$name."\">
                    ".error(form_error($name))."
                  </div>
                </div>"; 
      return $form;
